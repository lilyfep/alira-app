// app/(tabs)/buscar.tsx
// Búsqueda de Google Books para añadir libros a la colección
// Paridad con index.html de la web
...
export default function BuscarScreen() {
  ...
  const [adding, setAdding]     = useState<string | null>(null); // google_books_id añadiéndose
  ...
  const buscar = async () => {
    ...
    if (ok) {
      ...
      Alert.alert('Error', 'No se pudo buscar. Comprueba tu conexión.');
    }
  };

  const añadirAColeccion = async (item: SearchResult) => {
    setAdding(item.google_books_id);
    ...
    setAdding(null);
    if (ok) {
      setAdded(prev => new Set([...prev, item.google_books_id]));
    } else {
      Alert.alert('Error', data?.message || 'No se pudo añadir el libro.');
    }
  };

  const renderItem = ({ item }: { item: SearchResult }) => {
    const isAdded   = added.has(item.google_books_id);
    const isAdding  = adding === item.google_books_id;
      ...
      <View style={s.card}>
        ...
        <View style={s.info}>
          ...
          {item.anio_publicacion
            ? <Text style={s.year}>{item.anio_publicacion}</Text>
            : null}
          {item.descripcion
          ...
          <View style={s.actions}>
            <TouchableOpacity
              style={[s.addBtn, (isAdded || isAdding) && { opacity: 0.6 }]}
              onPress={() => añadirAColeccion(item)}
              disabled={isAdded || isAdding}
            >
              {isAdding
                ? <ActivityIndicator size="small" color={Colors.bg} />
                : <Text style={s.addBtnText}>
                    {isAdded ? '✓ Añadido' : '+ Mi colección'}
                  </Text>
              }
            </TouchableOpacity>
          </View>
  ...
  return (
    <SafeAreaView style={s.container}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerSub}>Añade libros a tu colección</Text>
      </View>
...
const s = StyleSheet.create({
  ...
  addBtn:       { backgroundColor: Colors.accent + 'cc', borderRadius: Radius.md,
                  paddingHorizontal: 14, paddingVertical: 8, borderWidth: 1, borderColor: Colors.accent + '55' },
